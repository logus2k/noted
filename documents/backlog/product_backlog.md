...
| **UX-4** | Citation badges — distinguish **document** (chunk) from **graph** (entity / edge / community) families | partial (staged); pending follow-ups | **Shipped 2026-05-06:** each badge now carries a small leading FontAwesome icon — `fa-file-lines` (green `#2e6e1f`) for chunks, `fa-share-nodes` (slate `#5a809c`) for the graph trio. Icon + pill wrapped in `inline-flex` (`.chat-citation-wrap`, gap 2px) so the pair never breaks across line wraps. Files: `frontend/js/ChatPanel.js` (`_renderCitations`), `frontend/css/chat-panel.css`. **Two-axis design:** icon = family (doc vs graph); pill hue = sub-type within graph. **Per-kind criteria** to keep when iterating: chunk → green family ("the evidence", source document); entity (`E:...`) → pale blue + dark blue text ("the thing", a graph node); edge (`R:...`) → pale amber + dark amber text ("the link", a relationship); community (`C\d+`) → pale purple + dark purple text ("the synthesis", a Leiden-cluster summary). Hover inverts each pill to a filled mid-tone of its own hue + white text. **Pending polish (not yet 100% done):** (a) chunk pill still uses mixed-hue rendering (pale green bg + blue text + slate border) — could be rebased to single-hue green family for full cross-kind consistency; (b) the chunk file-icon colour has been parked at `#2e6e1f`; explore alternative treatments (white on a green chip, etc.) when the chat-bubble background gets a distinct fill in a future skin pass. |
