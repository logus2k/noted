...
- **Automatic DVC hash injection into MLflow**: Whenever an MLflow run is created (explicit, auto-instrumented, or via Run Manager), noted's backend automatically injects the current DVC data hash as:
  - An MLflow run **tag** (`dvc.data_hash`) — for programmatic cross-referencing
  - An MLflow run **parameter** (`dvc_data_hash`) — for filtering and searching in the experiment table (this is the pattern taught in EMI Labs 2–3: `mlflow.log_param('dvc_data_hash', ...)`)
  The hash is computed from all `.dvc` pointer files in the project directory. No user action required.
- **MLflow experiment detail**: When viewing an experiment run (either via the MLflow tab or a future experiment panel), noted displays the Git commit SHA and the associated DVC file hash prominently in the run detail view.
...
- **Terminal as escape hatch**: When Git or DVC operations fail in non-standard ways (merge conflicts on `.dvc` files, lock contention, cache corruption), the UI shows a clear toast notification with an **"Open Terminal"** action button. The terminal opens pre-navigated to the project directory. noted does not attempt to build UI for every possible Git/DVC failure mode — the built-in xterm.js terminal handles the long tail of edge cases (see Architecture Principles, P6).
...
- **Lineage is automatic**: noted links DVC data versions to MLflow runs without requiring the user to manually log hashes. The DVC hash is injected as both an MLflow tag and parameter at run creation time.
