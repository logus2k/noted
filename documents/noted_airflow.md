...
- **Multiple DAGs per project**: A project can have multiple DAG files — for example, separate ingestion, training, and evaluation DAGs as in EMI Labs 5–6. All `.py` files in `dags/` and `pipelines/` are discovered and synced to Airflow. The Pipelines tree lists them all under the project node.
- **BashOperator + Hydra CLI support**: DAG files commonly use `BashOperator` to invoke Python scripts with Hydra CLI overrides (e.g., `python src/train.py model=lstm tracking.experiment_name=...`). noted's DAG editor provides syntax highlighting and validation for these patterns.
- **Dynamic task generation**: DAG files can scan the project's `conf/model/` directory to discover available model architectures and create per-model tasks dynamically (as in EMI Lab 5-P2). noted surfaces all dynamically generated tasks in the Pipeline Tab.
- **Jinja templating**: DAG files can use Jinja templating in BashOperator commands (e.g., `{{ params.start_date }}`). noted validates that referenced params exist in the DAG's params schema.
...
- **Run button on pipeline node**: Each DAG in the Pipelines tree has a Play button (▶). Clicking it opens a "Trigger Run" dialog showing: run ID (auto-generated), config parameters (pre-filled with defaults, editable), and a Hydra config override field.
- **Parameterized trigger form**: The trigger dialog pulls the `dag.params` schema from Airflow's REST API to present a dynamic form — users fill in labeled fields rather than raw JSON. Supports all Airflow `Param` types: text fields, date pickers (for `start_date`/`end_date` ranges), dropdowns (for model selection like `target_model: "all" | "lstm" | "gru" | "prophet"` as in EMI Lab 5-P2), and boolean toggles. Default values from `ParamsDict` are pre-filled.
...
- **Live run status in Pipelines tree**: The currently active run is shown under its DAG node with a progress indicator. Each task appears as a sub-node with its own status icon (queued / running / success / failed / skipped). Skipped tasks (e.g., model-selective tasks using exit code 99 as in EMI Lab 5-P2) are shown with an orange indicator. The tree auto-refreshes every 10 seconds during an active run.
