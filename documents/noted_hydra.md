...
**Hydra approach:** Define config groups in `conf/model/small.yaml` and `conf/model/large.yaml`. The training script (or notebook via `hydra.initialize`) loads the selected group. Switching presets is a config choice, not a code change. In the EMI course, this pattern is used extensively: `conf/model/lstm.yaml`, `conf/model/gru.yaml`, and `conf/model/prophet.yaml` define architecture-specific hyperparameters (hidden_size, num_layers, dropout for neural nets; changepoint_prior_scale for Prophet), while shared settings (optimizer, data paths, tracking) live in separate config groups.
...
- **Config selector in notebook bar**: A dropdown in the notebook's second bar (alongside the kernel selector) allows selecting the active Hydra config profile (e.g., `model=lstm`, `model=gru`, `model=prophet`). This selection is passed to the kernel environment so that notebook code using `hydra.initialize` picks it up automatically. For scripts using `@hydra.main`, the selected config is passed as CLI overrides when the script is invoked (either from a notebook cell or via an Airflow BashOperator task).
- **Resolved config preview**: A panel that shows the fully resolved Hydra config (after group selection and any overrides), giving users a clear picture of exactly what parameters will be used before they run any cells. This includes interpolated values from OmegaConf (e.g., `${api.target_location}` resolved to the actual location name).
