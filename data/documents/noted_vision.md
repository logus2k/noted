...
The model-serving container detects the `@champion` alias and hot-loads the model. A "Try It" center tab becomes active, showing an input form matching the model's expected schema. The researcher pastes sample weather data and gets a prediction inline.
