...
def _update_doc_with_review(
    ...
    if review_idx >= 0:
        ...
        if "_(empty" in section_body:
            # Replace the placeholder line with the new subsection,
            # preserving everything else (in practice the section is
            # empty so this is just clean-slate insertion).
            section_body = re.sub(
                r"\n_\(empty[^)]*\)_\n*",
                "\n",
                section_body,
...
async def setup_research_doc(
    ...
    # Workflow inputs land at state.inputs; the `inputs` arg here is
    # the per-step bundle (workflow_inputs + previous_step + ...).
    # Existing handlers (publish_skill, archive_tool, etc.) all read
    # from state.inputs; matching that pattern.
    wf_in = state.inputs or {}
...
async def research_session(
    ...
    Returns one of:
      ...
      {"status": "cap_reached", "iterations_run": N, "final_verdict": "..."}
    """
    from . import doc_events
    from .llm_dispatcher import dispatch_tool_calling
    from .loop import _suspend_for_hitl
    from .registry import get_workflow_registry

    # Workflow inputs land at state.inputs (same pattern as publish_skill,
    # archive_tool). The `inputs` arg is the per-step bundle.
    wf_in = state.inputs or {}
    notes_doc_id = (wf_in.get("notes_doc_id") or "").strip()
    criteria = list(wf_in.get("acceptance_criteria") or [])
    if not notes_doc_id:
    ...
    while True:
        ...
        if decision == "iterate":
            ...
            # The user's feedback (if any) is already in the doc's Review
            # Notes section — noted writes it there before signalling
            # resume. The researcher reads the doc on its next pass and
            # picks it up via the Review Notes section, same as it does
